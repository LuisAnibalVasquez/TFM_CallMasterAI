...
- [x] 5.1 Create `apps/frontend/src/features/tenants/services/tenantService.ts` — API client for CRUD endpoints
- [x] 5.2 Create `apps/frontend/src/features/tenants/hooks/useTenants.ts` — React hooks (useTenants, useCreateTenant, useUpdateTenant, useDeleteTenant) following project useState/useCallback pattern
- [x] 5.3 Create `apps/frontend/src/features/tenants/components/TenantForm.tsx` — create/edit form with AI config sections (sandbox + production API URL/Key fields)
- [x] 5.4 Create `apps/frontend/src/features/tenants/components/TenantList.tsx` — table with status toggle, delete button (guarded), temp password display modal
